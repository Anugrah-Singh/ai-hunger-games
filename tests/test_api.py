...
async def api_client(
    ...
    # ASGITransport deliberately does not manage application lifespan events.
    # Entering this app's lifespan starts its temporary database explicitly.
...
async def test_api_runs_and_exposes_a_simulated_generation(
    ...
    before_run = await api_client.get(f"/experiments/{experiment['id']}")
    assert before_run.status_code == 200
    assert before_run.json()["generation_count"] == 0
    assert len(before_run.json()["current_population"]) == 8
    assert before_run.json()["can_run"] is True
    assert before_run.json()["run_block_reason"] is None
    ...
    run = await api_client.post(
        f"/experiments/{experiment['id']}/generations",
    ...
    experiment_detail = await api_client.get(f"/experiments/{experiment['id']}")
    assert experiment_detail.status_code == 200
    assert experiment_detail.json()["generation_count"] == 1
    assert len(experiment_detail.json()["current_population"]) == 8
    ...
    rounds = generation_detail.json()["rounds"]
    assert len(rounds) == 8
    ...
    assert set(first_score) == {"candidate_id", "score"}
    assert "agent_id" not in first_score

    votes = await api_client.get(f"/generations/{game_id}/votes")
    assert votes.status_code == 200
    assert len(votes.json()) == 64
    assert all(
        set(vote) == {"voter_agent_id", "selected_candidate_id"}
        for vote in votes.json()
    )

    analysis = await api_client.get(f"/experiments/{experiment['id']}/analysis")
    assert analysis.status_code == 200
    assert analysis.json()["generation_count"] == 1
    assert "cautions" in analysis.json()
    assert {
        "personality",
        "generation_scores",
        "total_points",
    } <= set(analysis.json()["agent_performance"][0])
    assert {
        "periods",
        "eligible_voting_opportunities",
        "random_baseline_rate",
    } <= set(analysis.json()["vote_relationships"][0])
...
async def test_api_hides_unexpected_generation_failure(
    ...
    async def fail_run_generations(*_: object, **__: object) -> object:
    ...
    with caplog.at_level("ERROR", logger="ai_hunger_games.api"):
        response = await api_client.post(
            f"/experiments/{created.json()['id']}/generations",
            json={"generation_count": 1},
        )
