...
def create_app(
    ...
    async def create_experiment(
        ...
        settings_for_experiment: Settings = (
        ...
        try:
            experiment = await ExperimentRepository(
                ...
                provider_name=provider_name_for_settings(
    ...
    async def get_experiment(
        ...
        history = (
            await repository.load_experiment_history()
        )
        ...
        if history.generations:
            ...
            current_population = (
                [
                    _configured_agent_response(agent)
                    for agent in latest_population
                ]
            ...
            current_population = [
                _configured_agent_response(agent)
                for agent in definition.initial_agents
            ]
        else:
            current_population = []
        ...
        if (
            ...
            and experiment.provider_name is None
        ):
            run_block_reason = (
    ...
    async def run_experiment_generations(
        ...
        coordinator: ExperimentRunCoordinator = (
            request.app.state.run_coordinator
        )

        lock = await coordinator.try_acquire(
            experiment.id
        )
        ...
        generations_by_game_id = {
            generation.game_id: generation
            for generation in history.generations
        }

        return [
            _generation_summary_response(
                generations_by_game_id[
                    result.game_id
                ]
            )
    ...
    async def start_generation_run(
        ...
        starting_agents = (
            await _starting_agents_for_run(
                repository=repository,
        ...
        try:
            run = await GenerationRunRepository(
                session
            ).create_queued_run(
    ...
    async def get_generation(
        ...
        return _generation_detail_response(
            generation
        )
    ...
    async def get_generation_rounds(
        ...
        return [
            _round_response(round_snapshot)
            for round_snapshot
            in generation.rounds
        ]
...
async def _execute_generation_run(
    *,
    ...
    async with session_factory() as session:
        run_repository = (
            GenerationRunRepository(session)
        )
        ...
        try:
            ...
            if len(results) != 1:
                raise RuntimeError(
                    "The background generation did "
                    "not return exactly one result."
                )
            ...
            logger.exception(
                "Background generation run %s failed "
                "for experiment %s",
                run_id,
                experiment_id,
            )
            ...
            try:
                ...
                logger.exception(
                    "Could not mark generation run %s "
                    "as failed",
                    run_id,
                )
...
def _agent_response(
    ...
    return AgentResponse(
        ...
        personality=_personality_response(
            agent.personality
        ),
    )
...
def _configured_agent_response(
    ...
    return AgentResponse(
        ...
        personality=PersonalityResponse(
            ...
            answer_template=(
                agent.personality.answer_template
            ),
        ),
    )
...
def _generation_summary_response(
    ...
    return GenerationSummaryResponse(
        ...
        eliminated_agent_id=(
            generation.eliminated_agent_id
        ),
        replacement_agent=_agent_response(
            generation.replacement_agent
        ),
    )
        ...
        starting_agents=[
            ParticipantResponse(
                **_agent_response(
                ...
                was_eliminated=(
                    participant.was_eliminated
                ),
            )
            for participant
            in generation.starting_agents
        ],
        final_agents=[
            _agent_response(agent)
            for agent in generation.final_agents
        ],
        rounds=[
            _round_response(round_snapshot)
            for round_snapshot
...
def _round_response(
    ...
    candidate_id_by_agent_id = {
        answer.agent_id: answer.candidate_id
