...
class ExperimentRunCoordinator:
    """Serialize expensive runs within one process and experiment."""

    def __init__(self) -> None:
        self._locks: dict[int, asyncio.Lock] = {}

    async def try_acquire(self, experiment_id: int) -> asyncio.Lock | None:
        lock = self._locks.setdefault(experiment_id, asyncio.Lock())
...
ExperimentId = Annotated[int, ApiPath(ge=1)]
GameId = Annotated[int, ApiPath(ge=1)]
...
def create_app(
    ...
    async def get_experiment(
        ...
        experiment = await _get_experiment_or_404(session, experiment_id)
    ...
    async def list_generations(
        ...
        history = await _get_history_or_404(session, experiment_id)
        return [
    ...
    async def get_generation(
        ...
        generation = await _get_generation_or_404(session, game_id)
...
def _agent_response(agent: AgentSnapshot) -> AgentResponse:
...
def _configured_agent_response(agent: Agent) -> AgentResponse:
    return AgentResponse(
        agent_id=agent.id,
...
def _round_response(round_snapshot: RoundSnapshot) -> RoundResponse:
