...
export function Arena({ personalities, votes }: ArenaProps) {
  return (
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6">
        {personalities.map((personality) => (
          ...
          >
            ...
            {personality.alive && votes.length > 0 ? (
              <div className="mt-3 w-full rounded border border-amber-800/60 bg-amber-950/50 py-1 text-center text-xs font-semibold text-amber-300">
                {voteCount(votes, personality.id)} votes
              </div>
            ) : null}
